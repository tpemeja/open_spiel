...
const GameType kGameType{
    ...
    /*parameter_specification=*/
    {
        ...
        {"use_belote_rebelote", GameParameter(true)},
...
BeloteGame::BeloteGame(const GameParameters& params)
    : Game(kGameType, params),
      ...
      use_belote_rebelote_(ParameterValue<bool>("use_belote_rebelote")),
...
void BeloteState::EnterPlayPhase() {
  ...
  tricks_played_ = 0;
  team_points_ = {0, 0};
  if (use_belote_rebelote_) belote_team_ = FindBeloteTeam();
}

int BeloteState::FindBeloteTeam() const {
  ...
  for (Player player = 0; player < kNumPlayers; ++player) {
    const std::vector<int>& hand = hands_[player];
    bool has_king = absl::c_linear_search(hand, trump_king);
    bool has_queen = absl::c_linear_search(hand, trump_queen);
    if (has_king && has_queen) return TeamOf(player);
  }
  return -1;
}
...
void BeloteState::FinalizeScores() {
  ...
  int declarer_bonus =
      (belote_team_ == declarer_team_) ? kBeloteRebeloteBonus : 0;
  int other_bonus = (belote_team_ == other_team) ? kBeloteRebeloteBonus : 0;
...
std::string BeloteState::ToString() const {
  ...
  if (phase_ == Phase::kPlay || phase_ == Phase::kGameOver) {
    ...
    absl::StrAppend(&rv, "Team points: [", team_points_[0], ", ",
                    team_points_[1], "]\n");
    if (belote_team_ >= 0) {
      absl::StrAppend(&rv, "Belote/rebelote team: ", belote_team_, "\n");
    }
  }
  return rv;
}
