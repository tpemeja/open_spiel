...
// open_spiel/python/games/belote.py (registered there as "python_belote"):
...
// next player as dealer. This can in principle repeat indefinitely, so the
// number of redeals is capped by the "max_redeals" game parameter (default
// 10); if every deal keeps failing to produce a taker all the way up to that
...
// controlled via the "use_belote_rebelote" game parameter (on by default).
...
class BeloteState : public State {
 ...
 private:
  ...
  const bool use_belote_rebelote_;
  int belote_team_ = -1;
...
class BeloteGame : public Game {
 public:
  ...
  std::unique_ptr<State> NewInitialState() const override {
    return std::unique_ptr<State>(new BeloteState(
        shared_from_this(), dealer_, use_belote_rebelote_, max_redeals_));
  }
